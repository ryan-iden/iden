const sign_in_experiences = {
  empty_content_url_of_terms_of_use: '你啟用了“使用條款”，請添加使用條款網址。',
  ...
  duplicated_mfa_factors: '重複的多因素驗證要素。',
  email_verification_code_cannot_be_used_for_mfa:
    '當啟用電子郵件驗證用於登錄時，電子郵件驗證碼不能用於多因素驗證。',
  phone_verification_code_cannot_be_used_for_mfa:
    '當啟用SMS驗證用於登錄時，SMS驗證碼不能用於多因素驗證。',
  email_verification_code_cannot_be_used_for_sign_in:
    '當電子郵件驗證碼啟用用於多因素驗證時，不能用於登錄。',
  phone_verification_code_cannot_be_used_for_sign_in:
    '當SMS驗證碼啟用用於多因素驗證時，不能用於登錄。',
  adaptive_mfa_requires_mfa: '啟用自適應多因素驗證前必須先啟用多因素驗證。',
  adaptive_mfa_requires_non_skippable_policy:
    '自適應多因素驗證需要一個不可跳過的多因素驗證提示政策。使用PromptOnlyAtSignInMandatory或PromptAtSignInAndSignUpMandatory。',
  non_adaptive_mfa_requires_skippable_policy:
    '當自適應多因素驗證被禁用時，多因素驗證提示政策必須是可跳過的。不要使用PromptOnlyAtSignInMandatory或PromptAtSignInAndSignUpMandatory。',
  duplicated_sign_up_identifiers: '檢測到重複的註冊識別符。',
  missing_sign_up_identifiers: '主要註冊識別符不能為空。',
  invalid_custom_email_blocklist_format:
    '無效的自訂電子郵件封鎖清單項目：{{items, list(type:conjunction)}}。每個項目必須是有效的電子郵件地址或電子郵件域，例如：foo@example.com或 @example.com。',
