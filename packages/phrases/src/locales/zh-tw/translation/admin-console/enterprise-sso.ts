const enterprise_sso = {
  ...
  placeholder_description:
    'Logto提供了許多內置的企業身份提供者，同時你也可以使用安全斷言標記語言（SAML）和開放身分連接（OIDC）協議自定義自己的連接器。',
  create_modal: {
    title: '新增企業連接器',
  ...
  basic_info: {
    title: '在身分提供者中配置您的服務',
    description:
      '在您的 {{name}} 身份提供者中使用安全斷言標記語言（SAML） 2.0 建立新的應用集成。然後將以下值粘貼到其中。',
    saml: {
      acs_url_field_name: '斷言消費服務網址（回覆網址）',
      audience_uri_field_name: '受眾位址（服務提供者實體識別碼）',
      entity_id_field_name: '服務提供者（SP）實體識別碼',
      entity_id_field_tooltip:
        '服務提供者實體識別碼可以是任何字符串格式，通常使用位址或網址形式作為標識符，但這並不是強制的。',
      ...
      sign_auth_request_tooltip:
        'Logto會使用產生的憑證簽署安全斷言標記語言（SAML）驗證請求。僅當你的身份提供者已設定為驗證已簽署的請求時才啟用。',
    ...
    oidc: {
      redirect_uri_field_name: '重定向位址（回撥網址）',
      redirect_uri_field_description:
        '重新導向位址是在單一登入認證後使用者被重新導向的位置。請將此位址加入身分提供者的設定中。',
      redirect_uri_field_custom_domain_description:
        '如果你在Logto使用多個<a>自訂網域</a>，務必把所有對應的回呼位址加到身分提供者，確保單一登入在每個網域都能運作。\n\n預設的Logto網域(*.logto.app)一直有效；只有在你也希望支援該網域下的單一登入時才需要包含它。',
    },
  },
  attribute_mapping: {
    title: '屬性映射',
    description:
      '需要`id`和`email`來同步用戶檔案從身分提供者。 在您的身分提供者中輸入以下聲明名稱和值。',
  ...
  metadata: {
    ...
    metadata_format_url: '輸入元數據網址',
    metadata_format_xml: '上傳元數據可延伸標記語言（XML）檔案',
    metadata_format_manual: '手動輸入元數據細節',
    saml: {
      metadata_url_field_name: '元數據網址',
      metadata_url_description: '動態從元數據網址中獲取數據並保持憑證最新。',
      metadata_xml_field_name: '身分提供者元數據可延伸標記語言（XML）檔案',
      metadata_xml_uploader_text: '上傳元數據可延伸標記語言（XML）檔案',
      sign_in_endpoint_field_name: '登錄網址',
      idp_entity_id_field_name: '身分提供者實體識別碼（發行者）',
      certificate_field_name: '簽名憑證',
      certificate_placeholder: '複製並粘貼x509 憑證',
      certificate_required: '簽名憑證是必需的。',
    },
    oidc: {
      client_id_field_name: '客戶端識別碼',
