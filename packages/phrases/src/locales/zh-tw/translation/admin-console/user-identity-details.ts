const user_identity_details = {
  ...
  sso_account: {
    ...
    identity_id: '企業單點登錄身份識別碼',
    user_profile: '從企業單點登錄身份提供者同步的用戶資料',
  },
  token_storage: {
    title: '訪問令牌',
    description:
      '將 {{connectorName}} 的訪問和刷新令牌存儲在密鑰庫中。允許自動介面調用而無需重複用戶同意。',
  },
  access_token: {
    title: '訪問令牌',
    description_active:
      '訪問令牌是活躍且安全地存儲在密鑰庫中。你的產品可以使用它訪問 {{connectorName}} 介面。',
    description_inactive: '此訪問令牌為非活躍（如被撤銷）。用戶必須重新授權以恢復功能。',
    description_expired:
      '此訪問令牌已過期。在下一次介面請求時將自動使用刷新令牌進行更新。如果刷新令牌不可用，則需要用戶重新身份驗證。',
  },
  refresh_token: {
  ...
  delete_tokens: {
    ...
    confirmation_message:
      '你確定要刪除令牌嗎？Logto密鑰庫將移除存儲的 {{connectorName}} 訪問和刷新令牌。此用戶必須重新授權以恢復 {{connectorName}} 介面訪問。',
  },
  token_storage_disabled: {
    title: '此連接器的令牌存儲已禁用',
    description:
      '用戶目前只能使用 {{connectorName}} 進行登錄、鏈接帳戶或在每次同意流程中同步個人資料。要訪問 {{connectorName}} 介面並代表用戶執行操作，請在中啟用令牌存儲',
  },
};
