const webhook_details = {
  ...
  disable_reminder:
    '確定要重新啟用此事件回呼嗎？繼續操作後，將不會向端點網址發送超文字傳輸協定（HTTP）請求。',
  webhook_disabled: '事件回呼已成功停用。',
  webhook_reactivated: '事件回呼已成功重新啟用。',
  reactivate_webhook: '重新啟用事件回呼',
  delete_webhook: '刪除事件回呼',
  deletion_reminder:
    '您正在移除此事件回呼。刪除後，將不會向端點網址發送超文字傳輸協定（HTTP）請求。',
  deleted: '事件回呼已成功刪除。',
  settings_tab: '設置',
  ...
  settings: {
    ...
    settings_description:
      '事件回呼允許您即時接收特定事件的更新，通過向您的端點網址發送POST請求。這使您能夠根據接收到的新信息立即採取行動。',
    events: '事件',
    events_description: '選擇觸發Logto發送POST請求的觸發器事件。',
    name: '名稱',
    endpoint_url: '端點網址',
    signing_key: '簽名密鑰',
    signing_key_tip:
      '將Logto提供的密鑰添加到您的端點作為請求標頭，以確保事件回呼的有效載荷的真實性。',
    regenerate: '重新生成',
    ...
    test_webhook_description:
      '配置事件回呼，並使用每個選定事件的有效載荷示例進行測試，以驗證正確的接收和處理。',
