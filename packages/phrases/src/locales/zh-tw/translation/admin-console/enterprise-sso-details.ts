const enterprise_sso_details = {
  ...
  connector_logo_field_description: '每個圖像應該小於 500KB，僅支持SVG、PNG、JPG、JPEG。',
  ...
  email_domain_field_required: '需要郵箱域以啟用企業單一登入。',
  upload_saml_idp_metadata_info_text_url: '將身份提供者的元數據網址粘貼到此處以進行連接。',
  ...
  service_provider_property_title: '在身分提供者中進行配置',
  service_provider_property_description:
    '使用 {{protocol}} 在你的身份提供者中設置應用程序集成。輸入Logto提供的詳細信息。',
  ...
  idp_initiated_auth_config: {
    ...
    card_description:
      '用戶通常從你的應用程序使用服務提供者發起的單一登入流程開始身份驗證過程。除非絕對必要，否則不要啟用此功能。',
    ...
    auto_authentication_disabled_title: '重定向到客戶端以進行服務提供者發起的單一登入',
    auto_authentication_disabled_description:
      '推薦。將用戶重定向到客戶端應用程序以啟動安全的服務提供者發起的開放身分連接（OIDC）身份驗證。這將防止跨網站請求偽造（CSRF）攻擊。',
    auto_authentication_enabled_title: '直接使用身分提供者發起的單一登入進行登錄',
    auto_authentication_enabled_description:
      '成功登錄後，用戶將被重定向到指定的重定向位址，並帶有授權碼（無狀態和PKCE驗證）。',
    auto_authentication_disabled_app: '適用於傳統Web應用程序，單頁應用程序(SPA)',
    auto_authentication_enabled_app: '適用於傳統Web應用程序',
    idp_initiated_auth_callback_uri: '客戶端回調位址',
    idp_initiated_auth_callback_uri_tooltip:
      '客戶端回調位址，用於啟動服務提供者發起的單一登入身份驗證流程。 將ssoConnectorId作為查詢參數附加到位址中。（例如，https://your.domain/sso/callback?connectorId={{ssoConnectorId}}）',
    redirect_uri: '登錄後重定向位址',
    redirect_uri_tooltip:
      '成功登錄後重定向用戶的重定向位址。 Logto將使用此位址作為授權請求中的開放身分連接（OIDC）重定向位址。為身分提供者發起的單一登入身份驗證流程使用專用位址以提高安全性。',
    empty_redirect_uris_error: '該應用程序尚未註冊重定向位址。請先添加一個。',
    redirect_uri_placeholder: '選擇登錄後重定向位址',
  ...
  trust_unverified_email_tip:
    'Entra識別碼(OIDC)連接器不返回 `email_verified` 聲明，這意味著來自Azure的電子郵件地址不一定被驗證。默認情況下，Logto不會將未驗證的電子郵件地址同步到用戶配置文件中。如果你信任來自Entra識別碼目錄的所有電子郵件地址，僅需啟用此選項。',
  trust_unverified_email_tip_oidc:
    '開放身分連接（OIDC）連接器可能不會返回 `email_verified` 聲明，這意味著來自身份提供者的電子郵件地址不一定經過驗證。預設情況下，Logto不會將未驗證的電子郵件地址同步到使用者設定檔。只有在你信任來自身份提供者的所有電子郵件地址時才啟用此選項。',
  offline_access: {
    label: '刷新訪問令牌',
    description:
      '啟用Google `offline` 訪問以請求刷新令牌，允許你的應用程序在不重新授權用戶的情況下刷新訪問令牌。',
  },
};
