const organization_details = {
  ...
  add_members_to_organization_description:
    '通過搜索名稱、電子郵件、電話或用戶識別碼來查找適合的用戶。現有成員不會顯示在搜索結果中。',
  ...
  add_applications_to_organization_description:
    '通過搜索應用程式識別碼、名稱或描述來查找適合的應用程式。現有的應用程式不會顯示在搜索結果中。',
  ...
  search_application_placeholder: '按應用程式識別碼、名稱或描述搜尋',
  roles: '組織角色',
  ...
  search_user_placeholder: '按名稱、電子郵件、電話或用戶識別碼搜尋',
  at_least_one_user: '至少需要一個用戶。',
  organization_roles_tooltip: '分配給此組織中的 {{type}} 的角色。',
  custom_data: '自訂數據',
  custom_data_tip:
    '自訂數據是一個可用於存儲與組織相關的附加數據的JavaScript物件表示法（JSON）對象。',
  invalid_json_object: '無效的JavaScript物件表示法（JSON）對象。',
  branding: {
    name: '品牌',
    ...
    logo: '組織標誌',
    logo_tooltip:
      '你可以通過組織識別碼顯示此標誌在登錄體驗中；如果啟用了暗黑模式，則需要使用標誌的暗色版本。<a>了解更多</a>',
  },
  jit: {
    title: '即時配置',
    description:
    ...
    sso_enabled_domain_warning:
      '你已輸入一個或多個與企業單一登入相關的電子郵件域。擁有這些電子郵件的用戶將遵循標準單一登入流程，除非配置了企業單一登入配置，否則不會配置到此組織。',
    enterprise_sso: '企業單一登入配置',
    no_enterprise_connector_set:
      '您尚未設置任何企業單一登入連接器。請先添加連接器以啟用企業單一登入配置。<a>設置</a>',
    add_enterprise_connector: '添加企業連接器',
    enterprise_sso_description:
      '首次通過企業單一登入登錄的新用户或現有用户將自動加入組織。<a>了解更多</a>',
    organization_roles: '默認組織角色',
    organization_roles_description: '通過即時配置加入組織的用戶分配角色。',
  },
  mfa: {
    title: '多因素身份驗證(MFA)',
    tip: '當需要多因素驗證時，未設置多因素驗證的用戶在嘗試交換組織令牌時將會被拒絕。此設置不影響用戶身份驗證。',
