const add_on = {
  mfa_inline_notification:
    '多因素驗證是 {{planName}} 的每月 ${{price, number}} 加購項目。第一個月的費用將根據你的帳單週期按比例計算。<a>了解更多</a>',
  security_features_inline_notification:
    '啟用人機驗證、自定義鎖定體驗和其他高級安全功能——全部包含在每月僅需 ${{price, number}} 的附加組合中。',
  custom_domain:
  ...
  footer: {
    ...
    saml_apps:
      '額外的安全斷言標記語言（SAML）應用程式每月每個花費 <span>${{price, number}}</span>。第一個月的費用將根據你的帳單週期按比例計算。<a>了解更多</a>',
    third_party_apps:
      '額外的第三方應用程式每月每個花費 <span>${{price, number}}</span>。第一個月的費用將根據你的帳單週期按比例計算。<a>了解更多</a>',
    roles:
      '基於角色的訪問控制是Pro計劃中每月 <span>${{price, number}}</span> 的加購項目，具有無限角色。第一個月的費用將根據你的帳單週期按比例計算。<a>了解更多</a>',
  },
};
