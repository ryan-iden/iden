const user = {
  ...
  sso_identity_not_exist: '企业级单点登录账户尚未注册。',
  ...
  role_exists: '角色标识 {{roleId}} 已添加到此用户',
  invalid_role_type: '无效的角色类型，无法将机器到机器角色分配给用户。',
  suggest_mfa: '为提升账户安全性，我们建议你将多因素认证绑定到你的账户。',
  missing_mfa: '你需要在登录之前绑定额外的多因素认证。',
  totp_already_in_use: '基于时间的一次性密码（TOTP）已在使用中。',
  backup_code_already_in_use: '备用代码已在使用中。',
  password_algorithm_required: '密码算法是必需的。',
  password_and_digest: '不能同时设置明文密码和密码摘要。',
  personal_access_token_name_exists: '个人访问令牌名称已存在。',
  totp_secret_invalid: '无效的基于时间的一次性密码（TOTP）密钥。',
  wrong_backup_code_format: '备用代码格式无效。',
  username_required: '用户名是必需的标识符，你不能将其设置为null。',
  email_or_phone_required: '邮箱地址或手机号码是必需的标识符，至少需要一个。',
  last_sign_in_method_required: '你的账户至少需要保留一种可用的登录方式。',
  email_required: '邮箱地址是必需的标识符，你不能将其设置为null。',
  phone_required: '手机号码是必需的标识符，你不能将其设置为null。',
  enterprise_sso_identity_not_exists:
    '用户没有与指定的单点登录连接器标识 {{ ssoConnectorId }} 关联的企业身份。',
