const connectors = {
  ...
  placeholder_description:
    'Logto提供了许多广泛使用的社交登录连接器，同时你还可以使用标准协议创建自己的连接器。',
  save_and_done: '保存并完成',
  type: {
    email: '邮件连接器',
    sms: '短信连接器',
  ...
  guide: {
    ...
    connector_logo: '连接器标志',
    connector_logo_tip: '该标志将显示在连接器登录按钮上。',
    target: '身份提供商名称',
    ...
    target_tooltip:
      'Logto社交连接器的「target」指的是社交身份的「来源」。在Logto的设计里，我们不允许某一平台的连接器中有相同的「target」以避免身份的冲突。在添加连接器时，你需要格外小心，我们「不允许」用户在创建之后更改「target」的值。<a>了解更多</a>',
    target_conflict:
      '此「身份供应商名称」值与现有的 <span>name</span> 连接器相同。使用相同的身份供应商名称会导致不符合预期的登录行为，用户可能通过两个不同的连接器访问同一个帐户。',
    target_conflict_line2:
      '如果你想替换当前的连接器，并连接相同的身份提供商，以便先前的用户可以直接登录而无需重新注册，请先删除 <span>name</span> 连接器，再创建一个新的连接器并使用相同的「身份供应商名称」值。',
    target_conflict_line3: '如果你想连接一个新的身份验证提供程序，请修改「身份供应商名称」并继续。',
    config: '粘贴你的JavaScript对象表示法（JSON）代码',
    ...
    enable_token_storage: {
      title: '存储令牌以进行持久的接口访问',
      description:
        '将访问令牌和刷新令牌存储在密钥保险库（Secret Vault）中，无需反复获得用户同意即可自动调用接口。例如：让人工智能代理在持续授权下向 Google 日历添加事件。<a>了解如何调用第三方接口</a>',
    },
    callback_uri: '重定向地址（回调地址）',
    callback_uri_description:
      '重定向地址是社交授权后用户被重定向到的地址。请将显示的所有地址添加到身份提供商的配置中。',
    callback_uri_custom_domain_description:
      '如果你在Logto中使用多个<a>自定义域名</a>，务必将所有对应的回调地址都添加到身份提供商中，以确保社交登录在每个域名上都能正常工作。\n\n默认的Logto域名(*.logto.app)始终有效，只有在你也希望支持该域名下的登录时才需要包含它。',
    acs_url: '断言消费服务网址',
  },
  platform: {
    universal: '通用',
  ...
  create_form: {
    third_party_connectors:
      '集成第三方提供商以实现快速社交登录、社交账户绑定和接口访问。 <a>了解更多</a>',
