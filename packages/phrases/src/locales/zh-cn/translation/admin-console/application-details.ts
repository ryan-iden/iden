...
const application_details = {
  ...
  settings_description:
    '“应用”是注册的软件或服务，可以访问用户信息或代表用户执行操作。应用可以帮助Logto识别是谁在请求什么，并负责处理登录和授权。请填写认证所需的必填字段。',
  integration: '集成',
  integration_description:
    '使用由Cloudflare边缘网络提供动力的Logto安全工作者部署，实现世界各地的一流性能和 0ms冷启动。',
  service_configuration: '服务配置',
  service_configuration_description: '在您的服务中完成必要的配置。',
  session: '会话',
  endpoints_and_credentials: '端点和凭据',
  endpoints_and_credentials_description:
    '使用以下端点和凭据在应用程序中设置开放身份连接（OIDC）连接。',
  ...
  config_endpoint: 'OpenID 提供方配置端点',
  issuer_endpoint: '发行者端点',
  jwks_uri: '公钥集地址',
  authorization_endpoint: '授权端点',
  authorization_endpoint_tip: '进行鉴权与授权的端点。用于开放身份连接中的 <a>鉴权</a> 流程。',
  show_endpoint_details: '显示端点详细信息',
  hide_endpoint_details: '隐藏端点详细信息',
  logto_endpoint: 'Logto端点',
  application_id: '应用标识',
  application_id_tip: '应用的唯一标识，通常由Logto生成。等价于开放身份连接中的 <a>client_id</a>。',
  ...
  redirect_uri_tip:
    '在用户登录完成（不论成功与否）后重定向的目标地址。参见开放身份连接 <a>AuthRequest</a> 以了解更多。',
  mixed_redirect_uri_warning:
    '你的应用程序类型与至少一个重定向地址不兼容。这不符合最佳实践，我们强烈建议保持重定向地址的一致性。',
  wildcard_redirect_uri_warning:
    '通配符重定向地址不是标准开放身份连接（OIDC），可能会增加攻击面。请谨慎使用，尽可能使用精确的重定向地址。',
  post_sign_out_redirect_uri: '退出登录后重定向地址',
  post_sign_out_redirect_uris: '退出登录后的重定向 URI',
  post_sign_out_redirect_uri_placeholder: 'https://your.website.com/home',
  post_sign_out_redirect_uri_tip:
    '在用户退出登录后重定向的目标地址（可选）。在某些应用类型中可能无实质作用。',
  cors_allowed_origins: '跨源资源共享允许的来源',
  cors_allowed_origins_placeholder: 'https://your.website.com',
  cors_allowed_origins_tip:
    '所有重定向地址的来源将默认被允许。通常不需要对此字段进行操作。参见 <a>MDN文档</a> 以了解更多。',
  token_endpoint: '令牌端点',
  user_info_endpoint: '用户信息端点',
  enable_admin_access: '启用管理访问',
  enable_admin_access_label:
    '启用或禁用对管理接口的访问。启用后，你可以使用访问令牌代表该应用程序调用管理接口。',
  always_issue_refresh_token: '总是颁发刷新令牌',
  always_issue_refresh_token_label:
    '启用此配置将允许Logto始终颁发刷新令牌，无论身份验证请求中是否呈现 `prompt=consent`。 然而，除非必要，否则不推荐这样做，因为它与开放身份连接不兼容，可能会导致问题。',
  refresh_token_ttl: '刷新令牌有效期（天）',
  refresh_token_ttl_tip:
    '可用于请求新访问令牌的刷新令牌在过期之前的时间段。访问令牌请求将把刷新令牌的时效延长到此值。',
  rotate_refresh_token: '轮换刷新令牌',
  rotate_refresh_token_label:
    '启用后，当原先的刷新令牌的时效已经过去 70%，或者满足一定条件时，Logto将会为访问令牌请求发放新的刷新令牌。<a>了解更多</a>',
  rotate_refresh_token_label_for_public_clients:
    '启用后，Logto将为每个令牌请求颁发一个新的刷新令牌。<a>了解更多</a>',
  backchannel_logout: '后端通道注销',
  backchannel_logout_description: '配置开放身份连接后端通道注销端点，以及该应用是否需要会话。',
  backchannel_logout_uri: '后端通道注销地址',
  backchannel_logout_uri_session_required: '是否需要会话？',
  backchannel_logout_uri_session_required_description:
    '启用后，当使用 `backchannel_logout_uri` 时，RP需要在注销令牌中包含 `sid`（会话标识）声明，以标识RP与OP的会话。',
  token_exchange: '令牌交换',
  ...
  device_flow_notification:
    '此应用启用了开放授权（OAuth）2.0 设备授权流程，适用于输入受限的设备或无界面应用（如电视、命令行界面）。用户可在另一台设备上输入设备码或扫描二维码完成登录。<a>了解更多</a>',
  ...
  redirect_uri_required: '至少需要输入一个重定向地址。',
  app_domain_description_1: '随时使用由Logto提供支持的 {{domain}} 域名，永久有效。',
  ...
  custom_rules_tip:
    "以下是两种案例场景：<ol><li>仅对'/admin'和'/privacy'路由进行认证保护：^/(admin|privacy)/.*</li><li>排除JPG图像的认证：^(?!.*\\.jpg$).*$</li></ol>",
  ...
  third_party_settings_description:
    '将第三方应用程序与Logto集成为你的身份提供者(身份提供商)，使用开放身份连接（OIDC） / 开放授权（OAuth） 2.0，并提供用户授权同意界面。',
  ...
  custom_data_invalid: '自定义数据必须是有效的JavaScript对象表示法（JSON）对象',
  access_control: {
    name: '规则',
    title: '访问控制',
  ...
  branding: {
    ...
    terms_of_use_url: '应用程序使用条款网址',
    privacy_policy_url: '应用程序隐私政策网址',
  },
  permissions: {
    name: '权限',
    ...
    api_resource_permissions_assignment_form_title: '添加接口资源权限',
    user_data_permission_description_tips:
      '你可以通过“登录体验 > 内容 > 管理语言”修改个人用户数据权限的描述。',
    permission_description_tips:
      '当Logto作为第三方应用程序中的身份提供者（身份提供商）用于授权时，用户被要求授权时，此描述会显示在授权屏幕上。',
    user_title: '用户',
    ...
    oidc_title: '开放身份连接（OIDC）',
    oidc_description:
      '核心开放身份连接（OIDC）权限会自动为你的应用配置。这些作用域对认证至关重要，并且不会显示在用户授权屏幕上。',
    default_oidc_permissions: '默认开放身份连接（OIDC）权限',
    permission_column: '权限',
    guide_column: '指南',
    openid_permission: 'openid',
    openid_permission_guide:
      "用于访问开放授权（OAuth）资源时可选。\n用于开放身份连接（OIDC）认证时必需。授予访问身份令牌的权限，并允许访问 'userinfo_endpoint'。",
  ...
  roles: {
    ...
    assign_subtitle: '机器对机器应用必须拥有机器对机器类型的角色，才能访问相关的接口资源。',
  ...
  saml_idp_config: {
    title: '安全断言标记语言（SAML）身份提供商元数据',
    description: '使用以下元数据和证书配置你的应用中的安全断言标记语言（SAML）身份提供商。',
    metadata_url_label: '身份提供商元数据网址',
    single_sign_on_service_url_label: '单一登录服务网址',
    idp_entity_id_label: '身份提供商实体标识',
  },
  saml_idp_certificates: {
    title: '安全断言标记语言（SAML）签名证书',
    expires_at: '到期时间',
    finger_print: '指纹',
    status: '状态',
    active: '活跃',
  ...
  saml_idp_name_id_format: {
    title: '名称标识格式',
    description: '选择安全断言标记语言（SAML）身份提供商的名称标识格式。',
    persistent: '持久性',
    persistent_description: '使用Logto用户标识作为名称标识',
    transient: '临时性',
    transient_description: '使用一次性用户标识作为名称标识',
    unspecified: '未指定',
    unspecified_description: '使用Logto用户标识作为名称标识',
    email_address: '电子邮件地址',
    email_address_description: '使用电子邮件地址作为名称标识',
  },
  saml_encryption_config: {
    encrypt_assertion: '加密安全断言标记语言（SAML）断言',
    encrypt_assertion_description: '启用此选项后，安全断言标记语言（SAML）断言将被加密。',
    encrypt_then_sign: '先加密后签名',
    encrypt_then_sign_description:
      '启用此选项后，安全断言标记语言（SAML）断言将被加密然后签名；否则，安全断言标记语言（SAML）断言将被签名然后加密。',
    certificate: '证书',
    certificate_tooltip:
      '复制并粘贴从你的服务提供者获取的x509 证书以加密安全断言标记语言（SAML）断言。',
  ...
  saml_app_attribute_mapping: {
    name: '属性映射',
    title: '基础属性映射',
    description: '添加属性映射以从Logto同步用户资料到你的应用程序。',
