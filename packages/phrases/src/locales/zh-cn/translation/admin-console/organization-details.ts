const organization_details = {
  ...
  add_members_to_organization_description:
    '通过姓名、电子邮件、电话或用户标识搜索合适的用户。搜索结果中不显示现有成员。',
  ...
  add_applications_to_organization_description:
    '通过搜索应用标识、名称或描述来找到合适的应用。搜索结果中不显示现有应用。',
  ...
  search_application_placeholder: '按应用标识、名称或描述搜索',
  roles: '机构角色',
  ...
  search_user_placeholder: '按名称、电子邮件、电话或用户标识搜索',
  at_least_one_user: '至少需要一个用户。',
  organization_roles_tooltip: '{{type}}在此机构中分配的角色。',
  custom_data: '自定义数据',
  custom_data_tip:
    '自定义数据是一个JavaScript对象表示法（JSON）对象，可用于存储与机构关联的附加数据。',
  invalid_json_object: '无效的JavaScript对象表示法（JSON）对象。',
  branding: {
    name: '品牌',
    ...
    logo: '机构标志',
    logo_tooltip:
      '你可以传递机构标识以在登录体验中显示此标志；如果在全套登录体验设置中启用了深色模式，则需要深色版本的标志。<a>了解更多</a>',
  },
  jit: {
    title: '即时供应',
    description:
    ...
    sso_enabled_domain_warning:
      '你输入了一个或多个与企业单点登录关联的电子邮件域。使用这些电子邮件的用户将遵循标准的单点登录流程，除非配置了企业单点登录供应，否则不会被分配到该机构。',
    enterprise_sso: '企业单点登录供应',
    no_enterprise_connector_set:
      '你尚未设置任何企业单点登录连接器。首先添加连接器，以启用企业单点登录供应。<a>设置</a>',
    add_enterprise_connector: '添加企业连接器',
    enterprise_sso_description:
      '通过企业单点登录首次登录的新用户或现有用户将自动加入机构。<a>了解更多</a>',
    organization_roles: '默认机构角色',
    organization_roles_description: '通过即时供应加入机构时分配角色。',
  },
  mfa: {
    title: '多因素认证',
    tip: '当要求多因素认证时，没有配置多因素认证的用户在试图交换机构令牌时将被拒绝。此设置不影响用户身份验证。',
