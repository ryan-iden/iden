const application = {
  ...
  role_exists: '角色識別碼 {{roleId}} 已經被添加到此應用程式中。',
  ...
  cloudflare_unknown_error: '請求Cloudflare介面時發生未知錯誤',
  ...
  saml: {
    use_saml_app_api:
      '使用 `[METHOD] /saml-applications(/.*)?` 介面來操作安全斷言標記語言（SAML）應用程式。',
    saml_application_only: '此介面只適用於安全斷言標記語言（SAML）應用程式。',
    reach_oss_limit: '你不能創建更多的安全斷言標記語言（SAML）應用程式，因為已達到限制 {{limit}}。',
    acs_url_binding_not_supported: '僅支援使用 HTTP POST 綁定接收安全斷言標記語言（SAML）斷言。',
    acs_url_scheme_not_supported:
      '斷言消費者服務（ACS）網址僅支持超文字傳輸協定（HTTP）和超文字傳輸安全協定（HTTPS）協定。',
    can_not_delete_active_secret: '不能刪除活動密鑰。',
    no_active_secret: '未找到活動密鑰。',
    entity_id_required: '生成元數據需要實體識別碼。',
    name_id_format_required: '需要名稱識別碼格式。',
    ...
    acs_url_required: '需要斷言消費者服務網址。',
    private_key_required: '需要私鑰。',
    certificate_required: '需要證書。',
    invalid_saml_request: '無效的安全斷言標記語言（SAML）身份驗證請求。',
    auth_request_issuer_not_match:
      '安全斷言標記語言（SAML）身份驗證請求的發行者與服務提供者實體識別碼不匹配。',
    sp_initiated_saml_sso_session_not_found_in_cookies:
      '在瀏覽器儲存憑證（Cookie）中未找到服務提供者初始化的安全斷言標記語言（SAML）單一登入會話識別碼。',
    sp_initiated_saml_sso_session_not_found:
      '未找到服務提供者初始化的安全斷言標記語言（SAML）單一登入會話。',
