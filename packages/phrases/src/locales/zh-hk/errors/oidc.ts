const oidc = {
  aborted: '用戶終止了交互。',
  invalid_scope: '無效範圍: {{scope}}.',
  invalid_token: '權杖無效',
  invalid_client_metadata: '無效的客戶端元數據',
  insufficient_scope: '缺少權限範圍 `{{scope}}`。',
  invalid_request: '請求無效',
  invalid_grant: '授權請求無效',
  invalid_issuer: '無效的發行者。',
  invalid_redirect_uri: '無效返回鏈接, 該redirect_uri未被此應用注冊。',
  access_denied: '拒絶訪問',
  invalid_target: '請求資源無效',
  unsupported_grant_type: '不支持的grant_type',
  unsupported_response_mode: '不支持的response_mode',
  unsupported_response_type: '不支持的response_type',
  provider_error: '開放身分連接（OIDC）內部錯誤: {{message}}',
  server_error: '發生了未知的開放身分連接（OIDC）錯誤。請稍後重試。',
  provider_error_fallback: '發生了開放身分連接（OIDC）錯誤: {{code}}.',
  custom_claims_script_error: '自訂宣告腳本錯誤：{{error_description}}',
  key_required: '至少需要一個金鑰。',
  key_not_found: '未找到識別碼為 {{id}} 的金鑰。',
  only_previous_key_can_be_deleted: '僅可刪除上一個金鑰。',
  invalid_session_payload: '無效的會話有效載荷。',
  session_not_found: '未找到會話。',
  invalid_session_account_id: '會話帳戶識別碼不匹配。',
  failed_to_revoke_grant: '撤銷授權失敗。',
  failed_to_cleanup_session_authorization: '清理會話授權記錄失敗。',
};

export default Object.freeze(oidc);
