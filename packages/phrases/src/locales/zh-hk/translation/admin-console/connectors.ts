const connectors = {
  ...
  guide: {
    ...
    target_tooltip:
      'Logto社交連接器的「target」指的是社交身份的「來源」。在Logto的設計裡，我們不允許某一平臺的連接器中有相同的「target」以避免身份的衝突。在添加連接器時，你需要格外小心，我們「不允許」用戶在創建之後更改「target」的值。<a>了解更多</a>',
    target_conflict:
    ...
    enable_token_storage: {
      title: '存儲令牌以持續訪問介面',
      description:
        '將存取權杖和重新整理權杖儲存在密鑰保險庫（Secret Vault）中，無需用戶重複授權即可自動呼叫介面。例如：讓人工智能代理在持續授權下向 Google 日曆新增事件。<a>了解如何呼叫第三方介面</a>',
    },
    callback_uri: '重新導向位址（回調位址）',
    callback_uri_description:
      '重新導向位址是在社交授權後用戶被重新導向的位置。請將顯示的所有位址加入身分提供者的設定中。',
    callback_uri_custom_domain_description:
      '如果你在Logto使用多個<a>自訂網域</a>，請務必把所有對應的回調位址加入身分提供者，確保社交登入在每個網域都能運作。\n\n預設的Logto網域(*.logto.app)一直有效；只有在你也想支援該網域下的登入時才需要包含它。',
    acs_url: 'Assertion consumer service網址',
  },
  platform: {
    universal: '通用',
    web: '網頁',
  ...
  create_form: {
    third_party_connectors:
      '集成第三方提供商以快速社交登錄、社交帳戶鏈接和介面訪問。<a>了解更多</a>',
