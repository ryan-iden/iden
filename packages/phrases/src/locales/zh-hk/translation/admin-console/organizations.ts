const organizations = {
  ...
  title: '組織',
  subtitle:
    '組織通常用於軟體即服務（SaaS）或類似的多租戶應用程式，代表您的客戶團隊、組織或整個公司。組織作為B2B認證和授權的基礎要素。',
  ...
  organization_list_placeholder_text:
    '組織通常在軟體即服務（SaaS）或類似的多租戶應用程式中是最佳實踐。它們讓您開發應用程式，讓客戶可以建立和管理組織、邀請成員並分配角色。',
  ...
  guide: {
    ...
    introduction: {
      ...
      section_2: {
        ...
        description:
          '在多租戶軟體即服務（SaaS）應用程式中，多個組織通常共用相同的訪問控制模板，其中包括權限和角色。在Logto中，我們稱之為「組織模板」。',
        permission_description: '組織權限指授權在組織上下文中存取資源。',
        role_description_deprecated: '組織角色是可以分配給成員的組織權限的分組。',
        role_description: '組繇角色是可以分配給成員的組繇權限或介面權限的分組。',
      },
      section_3: {
        title: '我可以將介面權限分配給組織角色嗎？',
        description:
          '是的，您可以將介面權限分配給組織角色。Logto提供靈活性，有效管理您組織的角色，允許您在這些角色中包括組織權限和介面權限。',
      },
      section_4: {
        title: '交互示意圖，看看它們之間的關係',
        description:
          '讓我們舉個例子。John、Sarah屬於不同的組織，在不同組織的上下文中具有不同的角色。懸停在不同的模塊上，看看會發生什麼。',
      },
    },
    ...
    john: '約翰',
    john_tip:
      '約翰使用 "john@email.com" 作為唯一標識屬於兩個組織。他是組織A的管理員，同時是組織B的訪客。',
    sarah: '莎拉',
    sarah_tip: '莎拉使用 "sarah@email.com" 作為唯一標識屬於一個組織。她是組織B的管理員。',
  },
};
