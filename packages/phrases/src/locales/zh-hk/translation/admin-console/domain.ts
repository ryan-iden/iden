const domain = {
  ...
  update_endpoint_notice:
    '不要忘記在應用程序中更新社交連接器回調位址和Logto端點的域名，如果你想使用自定義域名進行功能。',
  error_hint: '確保更新您的網域名稱系統（DNS）記錄。我們將繼續每 {{value}} 秒檢查。',
  custom: {
    custom_domain: '自定義域名',
    ...
    add_domain: '添加域名',
    invalid_domain_format: '請提供有效的域名網址，至少有三部分，例如 "auth.domain.com."',
    verify_domain: '驗證域名',
    enable_ssl: '啟用安全通訊端層（SSL）',
    checking_dns_tip:
      '配置網域名稱系統（DNS）記錄後，此程序將自動運行，可能需要長達 24 個小時。不影響執行，可離開此界面。',
    enable_ssl_tip:
      '啟用安全通訊端層（SSL）將自動運行，可能需要長達 24 個小時。您可以在運行期間離開此界面。',
    generating_dns_records: '正在生成網域名稱系統（DNS）記錄...',
    add_dns_records: '請將以下網域名稱系統（DNS）記錄添加到網域名稱系統（DNS）提供者。',
    dns_table: {
      type_field: '類型',
      name_field: '名稱',
      value_field: '內容',
    ...
    deletion: {
      ...
      in_used_tip:
        '如果在之前的社交記錄或應用程式終端點中配置了此自定義域名，您需要先使用Logto默認域名"<span>{{domain}}</span>" 修改位址，使社交組件可以正常使用。',
      deleted: '成功刪除自定義域名！',
    },
