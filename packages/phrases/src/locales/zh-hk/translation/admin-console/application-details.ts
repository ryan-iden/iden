...
const application_details = {
  ...
  settings_description:
    '「應用程式」是已註冊的軟件或服務，可存取用戶資料或代替用戶操作。應用程式有助Logto辨識誰在申請甚麼，並處理登入和權限。請填寫必須欄位以完成驗證。',
  integration: '整合',
  integration_description:
    '使用Logto安全工作者，由Cloudflare的邊緣網路提供支持，實現頂級性能和全球 0 毫秒冷啟動。',
  service_configuration: '服務配置',
  service_configuration_description: '在您的服務中完成必要的配置。',
  session: '會話',
  endpoints_and_credentials: '端點與憑證',
  endpoints_and_credentials_description:
    '使用以下端點和憑證在您的應用程式中設置開放身分連接（OIDC）連接。',
  ...
  authorization_endpoint_tip: '進行驗證和授權的端點。用於開放身分連接中的 <a>驗證</a> 流程。',
  show_endpoint_details: '顯示端點詳情',
  hide_endpoint_details: '隱藏端點詳情',
  logto_endpoint: 'Logto端點',
  application_id: '應用程式識別碼',
  application_id_tip:
    '應用程式的唯一標識，通常由Logto生成。等同於開放身分連接中的 <a>client_id</a>。',
  ...
  redirect_uri_tip:
    '在使用者登入完成（不論成功與否）後重定向的目標位址。參見開放身分連接 <a>AuthRequest</a> 以瞭解更多。',
  mixed_redirect_uri_warning:
    '你的應用程式類型與至少一個重定向位址不兼容。這不符合最佳實踐，我們強烈建議保持重定向位址的一致性。',
  wildcard_redirect_uri_warning:
    '萬用字元重定向位址不是標準開放身分連接（OIDC），可能會增加攻擊面。請謹慎使用，盡可能使用精確的重定向位址。',
  post_sign_out_redirect_uri: '登出後重定向位址',
  post_sign_out_redirect_uris: '登出後重定向位址',
  post_sign_out_redirect_uri_placeholder: 'https://your.website.com/home',
  post_sign_out_redirect_uri_tip:
    '在使用者登出後重定向的目標位址（可選）。在某些應用程式類型中可能無實質作用。',
  cors_allowed_origins: '跨來源資源共用允許的來源',
  cors_allowed_origins_placeholder: 'https://your.website.com',
  cors_allowed_origins_tip:
    '所有重定向位址的來源將預設被允許。通常不需要對此欄位進行操作。參見 <a>MDN文檔</a> 以瞭解更多。',
  token_endpoint: '令牌端點',
  user_info_endpoint: '用戶信息端點',
  enable_admin_access: '啟用管理訪問',
  enable_admin_access_label:
    '啟用或禁用對管理介面的訪問。啟用後，你可以使用訪問權杖代表該應用程式調用管理介面。',
  always_issue_refresh_token: '始終發行刷新令牌',
  always_issue_refresh_token_label:
    '啟用此配置將允許Logto發行刷新令牌，無論是否在驗證請求中提供 `prompt=consent`。但是，除非必要，否則不建議這樣做，因為它不兼容開放身分連接，可能會引起問題。',
  refresh_token_ttl: '刷新令牌的有效期（天）',
  refresh_token_ttl_tip:
    '刷新令牌可用於在其過期之前請求新的訪問權杖的持續時間。訪問令牌將將默認的有效期限延長到此值。',
  rotate_refresh_token: '輪換刷新令牌',
  rotate_refresh_token_label:
    '啟用後，當原始有效期限達到 70% 或滿足某些條件時就可以在令牌請求中為刷新令牌發行新的刷新令牌。 <a>了解更多。</a>',
  rotate_refresh_token_label_for_public_clients:
    '啟用後，Logto將為每個令牌請求發行新的刷新令牌。 <a>了解更多</a>',
  backchannel_logout: '後台登出',
  backchannel_logout_description: '配置開放身分連接後台登出端點及是否需要會話用於此應用程式。',
  backchannel_logout_uri: '後台登出位址',
  backchannel_logout_uri_session_required: '是否需要會話？',
  backchannel_logout_uri_session_required_description:
    '啟用後，RP要求在登出令牌中包含 `sid`（會話識別碼）聲明，以便在使用 `backchannel_logout_uri` 時識別RP與OP的會話。',
  token_exchange: '權杖交換',
  ...
  app_domain_description_1: '隨時使用由Logto提供支持的 {{domain}} 網域，它是永久有效的。',
  ...
  custom_rules_tip:
    "以下是兩個案例：<ol><li>只保護需要驗證的路由 '/admin' 和 '/privacy'：^/(admin|privacy)/.*</li><li>排除JPG圖片不需要驗證：^(?!.*\\.jpg$).*$</li></ol>",
  ...
  third_party_settings_description:
    '將第三方應用程式與Logto作為身份提供者（身分提供者）集成，使用開放身分連接（OIDC） / 開放授權（OAuth） 2.0，包含用戶授權的同意屏幕。',
  ...
  custom_data_invalid: '自定義資料必須是一個有效的JavaScript物件表示法（JSON）對象',
  access_control: {
    name: '規則',
    title: '訪問控制',
  ...
  permissions: {
    ...
    permission_description_tips:
      '當Logto用作第三方應用程式的身份提供者（身分提供者）用於授權時，以及用戶被要求授權時，此描述將在同意屏幕上顯示。',
    user_title: '用戶',
  ...
  saml_idp_config: {
    title: '安全斷言標記語言（SAML）身分提供者元數據',
    description: '使用以下元數據和憑證在您的應用程式中配置安全斷言標記語言（SAML）身分提供者。',
    metadata_url_label: '身分提供者元數據網址',
    single_sign_on_service_url_label: '單一登入服務網址',
    idp_entity_id_label: '身分提供者實體識別碼',
  },
  saml_idp_certificates: {
    title: '安全斷言標記語言（SAML）簽名憑證',
    expires_at: '到期於',
    finger_print: '指紋',
    status: '狀態',
    active: '啟用',
  ...
  saml_idp_name_id_format: {
    title: '名稱識別碼格式',
    description: '選擇安全斷言標記語言（SAML）身分提供者的名稱識別碼格式。',
    persistent: '持久型',
    persistent_description: '使用Logto使用者識別碼作為名稱識別碼',
    transient: '臨時型',
    transient_description: '使用一次性使用者識別碼作為名稱識別碼',
    unspecified: '未指定',
    unspecified_description: '使用Logto使用者識別碼作為名稱識別碼',
    email_address: '電子郵件地址',
    email_address_description: '使用電子郵件地址作為名稱識別碼',
  },
  saml_encryption_config: {
    encrypt_assertion: '加密安全斷言標記語言（SAML）斷言',
    encrypt_assertion_description: '啟用此選項後，安全斷言標記語言（SAML）斷言將被加密。',
    encrypt_then_sign: '先加密再簽名',
    encrypt_then_sign_description:
      '啟用此選項後，安全斷言標記語言（SAML）斷言將被加密然後簽名；否則，安全斷言標記語言（SAML）斷言將被簽名然後加密。',
    certificate: '憑證',
    certificate_tooltip:
      '複製並貼上從您的服務提供商獲得的x509 憑證以加密安全斷言標記語言（SAML）斷言。',
  ...
  saml_app_attribute_mapping: {
    name: '屬性映射',
    title: '基本屬性映射',
    description: '添加屬性映射以將使用者配置檔從Logto同步到您的應用程式。',
