const enterprise_sso = {
  page_title: '企業單一登入',
  title: '企業單一登入',
  subtitle: '連接企業身份提供者並啟用單一登入。',
  create: '新增企業連接器',
  col_connector_name: '連接器名稱',
  col_type: '類型',
  col_email_domain: '電子郵件域名',
  placeholder_title: '企業連接器',
  placeholder_description:
    'Logto提供了許多內置的企業身份提供者來連接，同時您可以使用安全斷言標記語言（SAML）和開放身分連接（OIDC）協定創建自己的。',
  create_modal: {
    title: '新增企業連接器',
    text_divider: '或者您可以通過標準協議自定義您的連接器。',
    connector_name_field_title: '連接器名稱',
    connector_name_field_placeholder: '例如，{corp. name} - {identity provider name}',
    create_button_text: '創建連接器',
  },
  guide: {
    subtitle: '一個關於連接企業身份提供者的分步指南。',
    finish_button_text: '繼續',
  },
  basic_info: {
    title: '在身分提供者中配置你的服務',
    description:
      '在你的 {{name}} 身份提供者中通過安全斷言標記語言（SAML） 2.0 創建新應用集成。然後將以下值貼到其中。',
    saml: {
      acs_url_field_name: '斷言消費服務網址(回複網址)',
      audience_uri_field_name: 'Audience位址(服務提供者Entity識別碼)',
      entity_id_field_name: '服務提供者實體識別碼',
      entity_id_field_tooltip:
        '服務提供者實體識別碼可以是任何字符串格式，通常使用位址形式或網址形式作為標識符，但這不是必須的。',
      acs_url_field_placeholder: 'https://your-domain.com/api/saml/callback',
      entity_id_field_placeholder: 'urn:your-domain.com:sp:saml:{serviceProviderId}',
      sign_auth_request: '簽署驗證請求',
      sign_auth_request_tooltip:
        'Logto會使用產生的憑證簽署安全斷言標記語言（SAML）驗證請求。僅當您的身份提供者已設定為驗證已簽署的請求時才啟用。',
      signing_certificate_field_name: '請求簽署憑證',
      signing_keys_empty: '尚未產生簽署金鑰。',
      generate_signing_key: '產生新金鑰',
      signing_key_generated: '簽署金鑰已產生。',
      signing_key_activated: '簽署金鑰已啟用。',
      signing_key_deactivated: '簽署金鑰已停用。',
      signing_key_deleted: '簽署金鑰已刪除。',
      sign_auth_request_warning:
        '啟用後，請在下方產生簽署金鑰，並將其憑證註冊到您的身份提供者（並在該處開啟已簽署請求的驗證）。在憑證完成註冊之前，透過此連線登入將會失敗。',
    },
    oidc: {
      redirect_uri_field_name: '重定向位址（回調網址）',
      redirect_uri_field_description:
        '重新導向位址是在單一登入認證後用戶被重新導向的位置。請將此位址加入身分提供者的設定中。',
      redirect_uri_field_custom_domain_description:
        '如果你在Logto使用多個<a>自訂網域</a>，請務必把所有對應的回調位址加入身分提供者，確保單一登入在每個網域都能運作。\n\n預設的Logto網域(*.logto.app)一直有效；只有在你也想支援該網域下的單一登入時才需要包含它。',
    },
  },
  attribute_mapping: {
    title: '屬性映射',
    description:
      '同步用戶概要從身分提供者需要 `id` 和 `email`。在你的身分提供者中輸入以下聲明名稱和值。',
    col_sp_claims: '服務提供者（Logto）的值',
    col_idp_claims: '身份提供者的聲明名稱',
    idp_claim_tooltip: '身份提供者的聲明名稱',
  },
  metadata: {
    title: '配置身分提供者元數據',
    description: '從身份提供者配置元數據',
    dropdown_trigger_text: '使用其他配置方法',
    dropdown_title: '選擇你的配置方法',
    metadata_format_url: '輸入元數據網址',
    metadata_format_xml: '上傳元數據可延伸標記語言（XML）文件',
    metadata_format_manual: '手動輸入元數據細節',
    saml: {
      metadata_url_field_name: '元數據網址',
      metadata_url_description: '動態從元數據網址檢索數據並保持證書最新。',
      metadata_xml_field_name: '身分提供者元數據可延伸標記語言（XML）文件',
      metadata_xml_uploader_text: '上傳元數據可延伸標記語言（XML）文件',
      sign_in_endpoint_field_name: '登錄網址',
      idp_entity_id_field_name: '身分提供者實體識別碼（發行者）',
      certificate_field_name: '簽名證書',
      certificate_placeholder: '複製並粘貼x509 憑證',
      certificate_required: '需要簽名證書。',
    },
    oidc: {
      client_id_field_name: '客戶端識別碼',
      client_secret_field_name: '客戶端密鑰',
      issuer_field_name: '發行者',
      scope_field_name: '範圍',
      scope_field_placeholder: '輸入範圍（用空格分隔）',
    },
  },
};

export default Object.freeze(enterprise_sso);
